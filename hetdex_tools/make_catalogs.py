...
def create_source_catalog(version="3.0.0", update=False):
    ...
    if False:  # will hopefully remove all this soon
        ...
        print(
            "Final FOF analysis complete in {:3.2f} minutes \n".format((t1 - t0) / 60)
        )
...
def main(argv=None):
    ...
    wise_catalog = Table.read(
        op.join(config.imaging_dir, "catalogs", "wise-hetdexoverlap.fits")
    )
    source_table_coords = SkyCoord(
        source_table["ra_mean"], source_table["dec_mean"], unit="deg"
    )
    ...
    keep_wise = catalog_matches[
        "ra", "dec", "primary", "unwise_objid", "flux", "wise_dist"
    ]
    ...
    if False:  # doing above because we need to cross-match by coordinates for HDR3
        ...
        dtab = Table.read(
            op.join(config.imaging_dir, "catalogs", "desi-hetdex-v1.0.fits")
        )
        ...
        hetdex_coords = SkyCoord(
            ra=dtab["RA_HETDEX"], dec=dtab["DEC_HETDEX"], unit="deg"
        )
