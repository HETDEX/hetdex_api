...
Last Updated: 2020/11/4

@author: John Feldmeier (YSU)
"""

# TODO - will probably need to define a location of the RC3 csv file.
...
def read_detect(version='2.1.1'):
    """ Code stub to read in the survey table, taken 
    from 03-Detections_Database_and_API.ipynb"""
    #import numpy as np
    #import tables as tb
    #import matplotlib.pyplot as plt
...
def create_dummy_wcs(c1):
    ...
    gridsize = np.float64(3600.)  # image size in arcsec.  Somewhat arbitrary, but make large enough to enclose 
    gridstep = np.float64(1.)     # make the fake pixel scale one arsec per pixel for convenience
...
def create_ellreg(t, index, d25scale = 1.0):
    ...
    major = (t['SemiMajorAxis'][index]) * np.float64(2.) * d25scale * u.arcmin
    minor = (t['SemiMinorAxis'][index]) * np.float64(2.) * d25scale * u.arcmin
...
def show_ellipse_ring_source(t, index, coords, rings, outname, \ 
	image_survey='SDSSg', save_file=True, show_notebook=False): 
    ...
    radouter = rlimit * 2.1        # The size of the image displayed, slightly larger than 2 times
    nrings = rings.size
   ...
   # Create a region from the source we want to overplot     
    ...
    for reg in sourceCircles:
        pixel_region = reg.to_pixel(mywcs)  
        pixel_region.plot(ax=ax,edgecolor='purple', facecolor='purple', linewidth=5,fill=True) 
...
def show_ellipse_ring_detect(t, index, tdetect, rings, outname, image_survey='SDSSg', \
                                 save_file=True, show_notebook=False):
    ...
    source_scale = rings[-1]   # Use the outermost scale as the radius.  Make this somewhat larger than the 
...
def check_all_large_gal(tboth, tdetect, scale, verbose=False):
    ...
    for i in range(0, ngals):
        ...
        catalogmsk = d2d < rlimit      # Create Boolean mask of objects inside of this rlimit value
        if (len(catalogmsk) == 0):     # If there are no close sources, go on to the next galaxy
            break
            ...
            for j in range(0, len(near_sources)):
                ...
                isclose, name, zgal = onegal_flag_from_coords(coords, tboth, i, d25scale = scale)
...
test1  = False   # test1 - Check to see if RC3 catalog loaded properly, from local directory
test2  = False   # test2 - Check to see if RC3 catalog loaded properly, from config location - not yet
test3  = False   # test3 - load in the HDR 2.10 detections file, locally.  
...
test9  = True    # test9 - Plot all the HETDEX sources around a particular galaxy 
test10 = True    # test10 - search the entire detect list against the entire galaxy list


if (test1):
    print('Test 1 - Checking to see if RC3 catalog loaded properly, from local directory:')
    print(70*'-')
...
if(test3):
    print('\nTest 3 - Load in HDR detect table, from local file:')
    print(70*'-')
...
if (test4):
    ...
    sep = 2.0 * u.arcsec
    pas = np.float64([0.,180.,90.,270.]) * u.deg
...
if (test6):
    ...
    for i in range(0, nclose):
        ...
        isclose, name, zgal = onegal_flag_from_coords(coords, tboth, index, d25scale = 1.0)
        isclose2, name2, zgal2 = onegal_flag_from_coords(coords, tboth, index, d25scale = 2.0)
        print(idstring, coords.to_string(), dist, isclose, name, zgal, isclose2, name2, zgal2)
        numd25_1[i] = isclose       
        numd25_2[i] = isclose2
      
    print('Out of {0:d} objects closest, the number within D25, 2D25: {1:0.01f} {2:0.01f}'.format(nclose, np.sum(numd25_1), np.sum(numd25_2)))
...
if (test7):
    ...
    for i in range(0, nclose):
        ...
        dist = np.around(d2d[ind1[i]].arcmin, decimals = 2)
        isclose, name, zgal = gals_flag_from_coords(coords, tboth, d25scale=1.0, nmatches=3)
        isclose2, name2, zgal2 = gals_flag_from_coords(coords, tboth, d25scale=2.0, nmatches=3)
        print(idstring, coords.to_string(), dist, isclose, name, zgal, isclose2, name2, zgal2)
        numd25_1[i] = isclose       
        numd25_2[i] = isclose2
...
if (test8):
    ...
    show_ellipse_ring_source(tboth, ind1, coords, rings, '', save_file=False, show_notebook=True)
...
if (test9):
   ...
   # Example - plot all of the tdetect sources located around NGC 4707:
    ind1 = np.int(np.where(tboth['Name1'] == 'NGC 4707')[0])
    rings = np.array([0.5, 1.0, 1.5, 2.0, 2.5, 3.5])
    show_ellipse_ring_detect(tboth, ind1, tdetect, rings, '', save_file=False, show_notebook=True)
...
if (test10):
    ...
    for scale in d25scales:
        ...
        for j in range(0, 5):
            index = np.random.randint(ntot)
            print(tdetect['detectid'][index], outTest[index], outNames[index], outRedshifts[index])
