...
def FitCircularAperture(hdu=None, coords=None,
                        radius=1.5*u.arcsec,
    ...
    apcor_im = aper.to_pixel(w).to_mask(method='center').multiply(im_mask)/ aper.to_pixel(w).to_mask(method='center').multiply(np.ones_like(im))
    apcor = np.sum(apcor_im == 0)/np.sum( np.isfinite(apcor_im))
    
    # get median and standard deviation in background 
    mean_sigclip, median_sigclip, stddev_sigclip = sigma_clipped_stats(annulus_data_1d, mask=annulus_mask_1d)
    bkg_median = median_sigclip * aper.to_pixel(w).area * apcor
    bkg_stddev = stddev_sigclip * aper.to_pixel(w).area * apcor
    
    phottable = aperture_photometry(hdu[0].data, [aper, aper_annulus], 
                                    error=hdu[1].data, mask=im_mask, 
                                   wcs=wcs.WCS(hdu[0].header))
    if np.abs(bkg_median) > 2*bkg_stddev:
        flux = (phottable['aperture_sum_0'][0] - bkg_median) * u.Unit('10^-17 erg cm-2 s-1')
    else:
...
def get_flux_for_index(index, plot=True, convolve_image=True):
    ...
    if plot:
        ...
        flux, flux_err, bkg_stddev, apcor = FitCircularAperture(hdu=hdu, 
                                                         ...
                                                         plot=True, 
                                                         plottitle=plottitle)
        plt.text(2, 2, 'S/N={:3.2f}'.format(flux.value/bkg_stddev.value), size=18, color='w')
...
def get_flux_for_source(detectid, 
                        coords=None, 
                        shotid=None, 
                        wave=None, 
                        linewidth=None,
    ...
    try:
        hdu = make_narrowband_image(coords=coords_obj,
                                    shotid=shotid_obj,
                                    imsize=20*u.arcsec,
                                    pixscale=0.25*u.arcsec,
                                    convolve_image=convolve_image,
                                    dcont=50,
                                    wave_range = [wave_obj - 2.0 * linewidth_obj,
                                                  wave_obj + 2.0 * linewidth_obj],
...
def get_line_image(
    ...
    if detectid is not None:
        ...
        if wave_range is not None:
            ...
            print('You need to supply wave_range=[wave_start, wave_end] for collapsed image')
...
def fit_ellipse_for_source(
    ...
    print(s_r)
    try:
        r_n = -1. * s_r.tau #_0 #* const_arcsec_to_kpc
    except:
    ...
    lum_err = flux_err*4.* np.pi*lum_dist**2
    ...
    fig.suptitle('{}  ra={:3.2f}, dec={:3.2f}, wave={:5.2f}, z={:3.2f}, mf={}'.format(
        name,
        coords_obj.ra.value, 
    ...
    plt.text( 0.8, 0.9, cutout['instrument'] + cutout['filter'], 
             transform = ax4.transAxes, fontsize=20, color='w')
    plt.contour(hdu[0].data, transform=ax4.get_transform(w))
    plt.xlabel('RA')
    plt.ylabel('Dec')
    aper.plot(color='white', linestyle='dashed', linewidth=2, transform=ax4.get_transform(w))

    ax5 = fig.add_subplot(235)
    plt.errorbar(dist_kpc.value, 
                 iso_tab['intens'], 
    ...
    plt.text(0.4,0.6, 'L_lya={:3.2e}'.format(lum), transform = ax5.transAxes, fontsize=16)
...
#             color='blue', label='PSF seeing:{:3.2f}'.format(fwhm))
    ...
    moffat_1d = moffat_psf[0, xcen:-1, ycen] /moffat_psf[0, xcen, ycen] * iso_tab['intens'][0]
    r_1d = moffat_psf[1,xcen:-1, ycen]
    E.close()
    
    plt.plot(arcsec_to_kpc(pixscale*np.arange(80)), iso_tab['intens'][0]*(moffat_psf[0, 80:-1,80]/moffat_psf[0,80,80]), 
            linestyle='dashed', color='green', label='PSF seeing:{:3.2f}'.format(fwhm))
    plt.legend()
...
def get_sn_for_aperture_range(hdu, find_peak=False, r_list=np.arange(0.5, 3.1, 0.1), dtype=float):
    ...
    if find_peak:
        ...
        searchregion = r<3
        tbl = find_peaks(im, threshold, footprint=searchregion, mask=im_mask, wcs = wcs.WCS(hdu[0].header))
        if tbl is None:
            coords_peak = None
        else:
...
def fit_growing_aperture(detectid):
    ...
    if sn[-1] > 2:
        ...
        r_list, sn, coords_center = get_sn_for_aperture_range(hdu, r_list=np.arange(1.5, 8.2, 0.2, dtype=float))
        try:
            index_2sigma = np.max(np.where((sn > 2) & ( sn < sn_upper_limit) & np.isfinite(sn)))
            #print(index_2sigma)
            sn_2sigma = sn[index_2sigma]
            r_2sigma = r_list[index_2sigma]
    ...
    if np.isfinite(r_2sigma):
        plt.figure()
        plottitle=' {}  r_2sigma={:3.2}'.format(detectid, r_2sigma)
        flux_2sigma, flux_err_2sigma, bkg_stddev_2sigma, apcor_2sigma = FitCircularAperture(hdu=hdu, 
                                                                    coords=coords_center, 
                                                                    radius=r_2sigma*u.arcsec, 
                                                                    annulus=[r_2sigma*2, r_2sigma*3]*u.arcsec,
                                                                    plot=True, plottitle=plottitle)
        plt.text(2, 2, 'S/N={:3.2f}'.format(flux_2sigma.value/bkg_stddev_2sigma.value), size=18, color='w')
    ...
    flux_snmax, flux_err_snmax, bkg_stddev_snmax, apcor_snmax = FitCircularAperture(hdu=hdu, 
                                                                coords=coords_center, 
                                                                plot=False, 
    ...
    return r_2sigma, sn_2sigma, r_snmax, sn_max, flux_2sigma, flux_err_2sigma, bkg_stddev_2sigma, apcor_2sigma, flux_snmax, flux_err_snmax, bkg_stddev_snmax, apcor_snmax 


def make_im_catalog(detlist, filename='imflux.tab', save=True):
    
    t0 = time.time()
    imflux = Table(names=['detectid','r_2sigma', 'sn_2sigma', 'r_snmax',
                          'sn_max','flux_2sigma', 'flux_err_2sigma',
                          'bkg_stddev_2sigma', 'apcor_2sigma',
                          'flux_snmax', 'flux_err_snmax',
                          'bkg_stddev_snmax', 'apcor_snmax'])

    for det in detlist:
        r_2sigma, sn_2sigma, r_snmax, sn_max, flux_2sigma, flux_err_2sigma, bkg_stddev_2sigma, apcor_2sigma, flux_snmax, flux_err_snmax, bkg_stddev_snmax, apcor_snmax  = fit_growing_aperture(det)
        imflux.add_row([det, r_2sigma, sn_2sigma, r_snmax, sn_max, flux_2sigma, flux_err_2sigma, bkg_stddev_2sigma, apcor_2sigma, flux_snmax, flux_err_snmax, bkg_stddev_snmax, apcor_snmax])
