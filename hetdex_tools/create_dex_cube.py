...
def save_data_cube(shotid=None, ifuslot=None, filepath='datacubes', ffsky=False, F=None, E=None):
    """
    ...
    hdu = make_data_cube(shotid = shotid,
    ...
    data_hdu = fits.CompImageHDU(data, header=hdu[1].header, name='DATA', compression_type='RICE_1')
    error_hdu = fits.CompImageHDU(error, header=hdu[2].header, name='ERROR', compression_type='RICE_1')
    mask_hdu = fits.CompImageHDU(mask, header=hdu[3].header, name='MASK', compression_type='RICE_1')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--filepath', type=str, default='datacubes', help='Directory to save output files')
    parser.add_argument('--ffsky', action='store_true', help='Apply fiber flat sky subtraction')
    ...
    for ifuslot in np.unique(F.ifuslot):
        save_data_cube(shotid=args.shotid, ifuslot=ifuslot, filepath=output_dir, ffsky=args.ffsky, F=F, E=E)
