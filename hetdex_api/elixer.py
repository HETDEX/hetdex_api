...
def read_elixer_catalogs(fibfn, catfn):
    ...
    data = [np.array(detectid), np.array(ra), np.array(dec), np.array(ew_obs), np.array(plae_poii_hetdex)]
    colnames = ['detectid', 'ra', 'dec', 'ew_obs', 'plae_poii_hetdex']
    elixer_table = Table(data, names=colnames)

    #then read in cat.txt 
    with open(catfn, "r") as f:
        for line in f:
            ...
            if (toks[32] is not None) and (toks[32].lower() != 'none'):
                ...
                plae_poii.append(-1)
            #print('Could not ingest %s' % toks[1])

    colnames2 = ['detectidcat', 'ra_match', 'dec_match', 'dist_match', 'filt'
                 'cat_mag','plae_poii']
    data2 = [np.array(ra_match), np.array(dec_match), np.array(dist), np.array(filt),
             np.array(cat_mag), np.array(plae_poii)]
