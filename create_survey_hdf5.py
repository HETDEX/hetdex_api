...
Note: requires makemaster.sh to collect info from Karl's /work/00115/gebhardt/maverick/gettar
...
class Survey(tb.IsDescription):
    ...
    fwhm = tb.Float32Col()
    response_4540 = tb.Float32Col() #throughput at 4540A
...
for idx in np.arange(np.size(shotarray)):
    ...
    if np.size(sel)==1:
        ...
        print "Weird: More than one RA/DEC/PA in value for master_sci['SHOT'][idx]"
    ...
    if np.size(sel1)==1: 
        ...
        print "Weird: More than one FWHM value for master_sci['SHOT'][idx]"
        print "Saving the first FWHM value only. Make sure this is ok."
        row['fwhm'] = np.float(master_fwhm['FWHM'][sel1][0])       
    else:
        row['fwhm'] = np.nan
        
    if row['fwhm'] < 0: row['fwhm']= np.nan          

    sel2=np.where( (master_tp['DATE']==row['date']) * (master_tp['OBSID']==row['obsid']) )
    if np.size(sel2)==1:
        row['throughput']=master_tp['TP'][sel2]
    elif np.size(sel2)>1:
        print "Weird: More than two TPs found in mastertp file for", row['shotid']
        row['throughput']=master_tp['TP'][sel2][0]
    else:
...
for idx in np.arange(np.size(master_sci['SHOT'])):
    ...
    fitsfile=op.join(path_astrometry,str(row['date'])+'v'+str(row['obsid']).zfill(3),str(row['date'])+'v'+str(row['obsid']).zfill(3)+'fp_'+row['expn']+'.fits')
